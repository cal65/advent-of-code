...
def read_df(file):
    ...
    password_df = pd.DataFrame.from_records(
        split_tuples, columns=["policy", "password"]
    )
    return password_df
...
def day2(file):
    ...
    password_df["count"] = password_df.apply(
        lambda x: x.password.count(x.letter), axis=1
    )
    password_df["pass"] = (password_df["count"] >= password_df["min"]) & (
        password_df["count"] <= password_df["max"]
    )
...
def day2b(file):
    password_df = read_df(file)
    password_df = transform_df(password_df, "policy")
    password_df["position_1"] = password_df.apply(
        lambda x: x.password[x["min"] - 1] == x.letter, axis=1
    )
    password_df["position_2"] = password_df.apply(
        lambda x: x.password[x["max"] - 1] == x.letter, axis=1
    )
...
def traverse(grid, start_row, start_col, x, y, tree_char="#"):
    ...
    if (start_row + y > grid_row) | (start_col + x > grid_col):
        ...
        return value + traverse(
            grid, start_row + y, start_col + x, x, y, tree_char=tree_char
        )
...
def count_valid_passports(file):
    ...
    return sum(
        [check_passport(passport, required_fields) for passport in passport_list]
    )
...
def day_11(file):
    d11 = pd.read_table(file, header=None, skip_blank_lines=False)[0]
    d11 = convert_layout(d11)
    return d11
...
def convert_layout(layout):
    layout_lists = [list(lay) for lay in layout]
    layout_array = np.array(layout_lists)
    return layout_array
...
def count_adjacent(layout, row, column):
    def get_seat(layout, row, column):
        if (
            (row < 0)
            or (column < 0)
    ...
    adjacent = (
        int(get_seat(layout, row - 1, column - 1) == "#")
        + int(get_seat(layout, row - 1, column) == "#")
        + int(get_seat(layout, row - 1, column + 1) == "#")
        + int(get_seat(layout, row, column - 1) == "#")
        + int(get_seat(layout, row, column + 1) == "#")
        + int(get_seat(layout, row + 1, column - 1) == "#")
        + int(get_seat(layout, row + 1, column))
        + int(get_seat(layout, row + 1, column + 1) == "#")
